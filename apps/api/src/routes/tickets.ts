...
ticketsRouter.post('/', async (request, response, next) => {
  try {
    ...
    // Perform AI analysis and multi-factor priority scoring synchronously before creation
    ...
    const scoring = calculateAutoPriority({
      ...
      customerTier: user.tier ?? CustomerTier.STANDARD,
    });

    const finalPriority = input.priority ?? scoring.autoPriority;
    ...
    const createdTicket = await prisma.ticket.create({
      data: {
        ...
        status: analysis.autoResolvable ? TicketStatus.AUTO_RESOLVED : TicketStatus.OPEN,
    ...
    // Create initial AI response reply if auto-reply generated
    ...
    await prisma.auditEvent.create({
      data: {
        ticketId: createdTicket.id,
        actorId: user.id,
        action: analysis.autoResolvable ? 'ticket.auto_resolved' : 'ticket.created',
...
ticketsRouter.post('/:id/replies', async (request, response, next) => {
  try {
    ...
    if (!isStaff) {
      void enrichTicketWithAi(existing.id, existing.subject, input.body);
    }
...
function serializeTicket(ticket: TicketWithRelations, isStaff: boolean) {
  return {
    ...
    autoPriority: ticket.autoPriority,
    category: ticket.category,
    aiSummary: ticket.aiSummary,
    sentiment: ticket.sentiment,
    sentimentScore: ticket.sentimentScore,
    urgencyKeywords: ticket.urgencyKeywords,
